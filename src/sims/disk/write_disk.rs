...
pub fn load_elf_to_mem(
    ...
    if &buf[0..4] != b"\x7fELF" {
        return fail(format!(
            "[SimDRAM_bb] Not a valid ELF file: {}",
            path.display()
        ));
    }
    ...
    let phoff = usize::try_from(read_u64(&buf, 32, "e_phoff")?).map_err(|_| {
        Whatever::without_source("[SimDRAM_bb] e_phoff does not fit usize".to_string())
    })?;
    ...
    let mem_size = u64::try_from(data.len()).map_err(|_| {
        Whatever::without_source("[SimDRAM_bb] memory size does not fit u64".to_string())
    })?;
    let mem_end = mem_base.checked_add(mem_size).ok_or_else(|| {
        Whatever::without_source("[SimDRAM_bb] memory range overflow".to_string())
    })?;
    ...
    for i in 0..phnum {
        let ph = phoff
            ...
            .ok_or_else(|| {
                Whatever::without_source("[SimDRAM_bb] program header offset overflow".to_string())
            })?;
        ...
        let seg_end = p_paddr.checked_add(p_memsz).ok_or_else(|| {
            Whatever::without_source("[SimDRAM_bb] segment range overflow".to_string())
        })?;
        ...
        let dst_start = usize::try_from(p_paddr - mem_base).map_err(|_| {
            Whatever::without_source("[SimDRAM_bb] segment offset does not fit usize".to_string())
        })?;
        let file_sz = usize::try_from(p_filesz).map_err(|_| {
            Whatever::without_source("[SimDRAM_bb] segment filesz does not fit usize".to_string())
        })?;
        let mem_sz = usize::try_from(p_memsz).map_err(|_| {
            Whatever::without_source("[SimDRAM_bb] segment memsz does not fit usize".to_string())
        })?;
        let dst_file = mem_range(data.len(), dst_start, file_sz, "segment destination")?;
        data[dst_file].copy_from_slice(&buf[src]);

        if mem_sz > file_sz {
            let zero_start = dst_start.checked_add(file_sz).ok_or_else(|| {
                Whatever::without_source("[SimDRAM_bb] zero fill offset overflow".to_string())
            })?;
            let dst_zero = mem_range(
                data.len(),
                zero_start,
                mem_sz - file_sz,
                "segment zero fill",
        ...
        loaded = loaded.checked_add(file_sz).ok_or_else(|| {
            Whatever::without_source("[SimDRAM_bb] loaded byte count overflow".to_string())
        })?;
    }

    println!(
        "[SimDRAM_bb] Loaded ELF '{}': {} bytes",
        path.display(),
        loaded
    );
    Ok(loaded)
...
fn file_range(buf: &[u8], at: u64, len: u64, name: &str) -> Result<Range<usize>, Whatever> {
    let at = usize::try_from(at).map_err(|_| {
        Whatever::without_source(format!("[SimDRAM_bb] {name} offset does not fit usize"))
    })?;
    let len = usize::try_from(len).map_err(|_| {
        Whatever::without_source(format!("[SimDRAM_bb] {name} size does not fit usize"))
    })?;
