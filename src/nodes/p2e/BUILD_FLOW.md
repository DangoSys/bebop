...
├── vvacDir/
│   ├── runtimeDir/
│   │   ├── lib/lib_arm/libvCtb.so
│   │   ├── include/ICtb.h
│   │   └── rtcfg/
│   └── vvac_by_mod/filelist
├── fpgaCompDir/
│   └── bitstream.bit
└── rev_1/
    └── xepic_vvac_top.vm
...
├── vvacDir/
│   ├── runtimeDir/
│   │   ├── lib/lib_arm/libvCtb.so  ← Rust FFI 链接这个
│   │   ├── include/                 ← 头文件（参考用）
│   │   └── rtcfg/                   ← P2ESimulator::new() 需要
│   └── vvac_by_mod/filelist         ← vsyn 输入
├── fpgaCompDir/
│   └── bitstream.bit                ← VdbgSession 需要
└── rev_1/
    └── top.vm                       ← vcom 输入
```
...
### 2. vcom_compile.tcl（vcom 配置）
```tcl
design_read -netlist ./rev_1/top.vm
netlistmacro -instance top.ddr_wrapper -package ${nnmf_path}/xepic_ddr4_dc1 -location 0.A -channel 0
design_load -top top
emulator_spec -add "file hw-config.hdf"
read_net -add {signal_name}   # 从 RTL 读取
write_net -add {signal_name}  # 向 RTL 写入
design_generation
```

### 3. hw-config.hdf（硬件配置）
- FPGA 板卡配置文件
- 由芯华章提供
...
1. ✅ 理解构建流程
2. 🔄 实现 `BitstreamBuilder`（调用 vvac/vsyn/vcom/make）
3. ⏳ 实现 `VdbgSession`（TCL 脚本生成和执行）
4. ⏳ 实现 `P2ESimulator`（DPI-C FFI）
5. ⏳ 测试完整流程
