...
//  3. Initialize Stage 2: Initialize hart state after Rust has loaded the
//  workload into BEMU memory.
...
extern "C" {
uint64_t handle_syscall_ffi(void *, uint64_t, uint64_t, uint64_t, uint64_t,
...
extern "C" void *current_bemu_state() { return current_emu_state; }
...
void *spike_create_raw(const char *isa, size_t procs, size_t hart_id,
                       uint8_t *mem_ptr, size_t mem_size, const char *log_path,
                       uint8_t *uart_ptr, uint8_t *clint_ptr, uint8_t *plic_ptr,
                       void *emu_state, bool profile_enabled) {
  init_crash_handler();
  ensure_buckyball_registered();

  if (procs != 1) {
    fprintf(stderr,
            "[ERROR] only one Spike hart is supported for now, got %zu\n",
            procs);
    fflush(stderr);
    return nullptr;
  }
  ...
  ctx->btif =
      new BTIF(mem_ptr, mem_size, uart_ptr, clint_ptr, plic_ptr, isa, hart_id);
  const char *final_isa = ctx->btif->get_cfg().isa;
  ctx->proc =
      new processor_t(final_isa, "MSU", &ctx->btif->get_cfg(), ctx->btif,
                      hart_id, false, ctx->log_file, std::cerr);
  ctx->proc->set_max_vaddr_bits(39);
  ctx->proc->reset();

  if (std::strstr(final_isa, "xbuckyball") != nullptr &&
      !check_buckyball_mounted(ctx)) {
    destroy_context(ctx);
    return nullptr;
  }
...
//===----------------------------------------------------------------------===//
bool spike_init_hart_raw(void *raw_ctx, uint64_t entry,
                         uint64_t trap_handler_addr, uint64_t satp,
                         uint64_t initial_sp, uint64_t initial_a0,
                         uint64_t initial_a1, uint64_t initial_a2,
                         const uint64_t *tp_value_ptr, bool pk) {
...
static bool finish_if_requested(spike_context_t *ctx) {
  if (ctx->finished || ctx->btif->exit_requested ||
      should_exit_ffi(ctx->emu_state)) {
    ctx->finished = true;
    ctx->exit_code = ctx->btif->exit_requested
                         ? ctx->btif->exit_code
                         : get_exit_code_ffi(ctx->emu_state);
    return true;
  }
  return false;
}
...
static bool is_ecall_cause(reg_t cause) {
  return cause == CAUSE_USER_ECALL || cause == CAUSE_SUPERVISOR_ECALL ||
         cause == CAUSE_MACHINE_ECALL;
}

static uint64_t handle_guest_syscall(spike_context_t *ctx) {
  return handle_syscall_ffi(ctx->emu_state, ctx->state->XPR[17],
                            ctx->state->XPR[10], ctx->state->XPR[11],
                            ctx->state->XPR[12], ctx->state->XPR[13],
                            ctx->state->XPR[14], ctx->state->XPR[15]);
}
...
static int handle_syscall_magic_pc(spike_context_t *ctx) {
  ...
  if (!is_ecall_cause(mcause)) {
    fprintf(stderr,
            "[ERROR] Non-ecall trap reached syscall handler: mcause=%ld "
            "mepc=0x%lx tval=0x%lx pc=0x%lx tp=0x%lx a0=0x%lx a5=0x%lx\n",
            mcause, ctx->state->csrmap[CSR_MEPC]->read(),
...
static bool skip_invalid_compressed_jump(spike_context_t *ctx) {
  uint16_t insn16 = 0;
  if (ctx->state->pc < DRAM_BASE ||
      ctx->state->pc + 2 > DRAM_BASE + ctx->mem_size) {
    return false;
  }
...
static int fail_unhandled_trap(spike_context_t *ctx, trap_t &trap) {
  fprintf(stderr,
          "[ERROR] Unhandled trap: cause=%ld mcause=%ld mepc=0x%lx tval=0x%lx "
          "pc=0x%lx\n",
          trap.cause(), ctx->state->csrmap[CSR_MCAUSE]->read(),
          ctx->state->csrmap[CSR_MEPC]->read(),
          ctx->state->csrmap[CSR_MTVAL]->read(), ctx->state->pc);
  fflush(stderr);
  ctx->finished = true;
  ctx->exit_code = 1;
...
static int step_instructions(spike_context_t *ctx, uint64_t count) {
  ...
  if (!ctx->profile_enabled) {
    ...
    const reg_t retired =
        ctx->state->csrmap[CSR_MINSTRET]->read() - retired_before;
    ...
    const reg_t value =
        (pending & 1 ? MIP_MSIP : 0) | (pending & 2 ? MIP_MTIP : 0);
...
int spike_step_raw(void *raw_ctx, uint64_t count) {
  ...
  const uint64_t pk_trap_handler = DRAM_BASE + ctx->mem_size - 0x2000;
  if (ctx->pk_mode && ctx->state->pc == pk_trap_handler &&
      step_instructions(ctx, 1) < 0) {
    return -1;
  }
...
bool spike_finished_raw(void *raw_ctx) {
  ...
  return ctx->finished || ctx->btif->exit_requested ||
         should_exit_ffi(ctx->emu_state);
}
