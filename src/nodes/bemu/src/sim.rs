...
impl BemuSim {
    fn config(cli: BemuCli) -> Result<Self, Whatever> {
        let (log, isa, procs, mem_size, _batch, _help) = parse_args(cli.args)?;
        Ok(Self {
            elf: cli.elf,
            log,
            isa,
    ...
    fn run(self) -> Result<(), Whatever> {
        ...
        run_spike(&self.isa, self.procs, mem_mb, elf_path, log_path).whatever_context("spike execution failed")
    }
}
