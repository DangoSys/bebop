...
impl<T: InferenceTransport + 'static> Service<T> {
    ...
    fn handle_connection(&self, mut stream: TcpStream) -> std::io::Result<()> {
        ...
        loop {
            ...
            if buffer.len() > 64 * 1024 {
                return write_response(&mut stream, 413, json!({"error": "headers too large"}));
            }
        }
    ...
    fn route(&self, method: &str, path: &str, authorization: &str, body: &[u8]) -> Result<(u16, Value), (u16, Value)> {
        ...
        if authorization != format!("Bearer {}", self.api_key) {
            return Err((401, json!({"error": "invalid api key"})));
        }
        if path == "/v1/models" && method == "GET" {
            return Ok((200, json!({"data": self.registry.models()})));
        }
        let input: Value = serde_json::from_slice(body).map_err(|_| (400, json!({"error": "invalid JSON body"})))?;
        let model = input
            .get("model")
            .and_then(Value::as_str)
            .ok_or((400, json!({"error": "model is required"})))?;
        if !self.registry.contains(model) {
            return Err((404, json!({"error": format!("model is not registered: {model}")})));
        }
        let request = match (method, path) {
            ("POST", "/v1/vision/classify") => InferenceRequest::ImageClassification {
                model: model.into(),
                input,
            },
            ("POST", "/v1/chat/completions") => InferenceRequest::Chat {
                model: model.into(),
        ...
        self.transport
            ...
            .map(|result| {
                (
                    200,
                    serde_json::to_value(result).unwrap_or_else(|_| json!({"error": "serialization failure"})),
                )
            })
            .map_err(|error| (502, json!({"error": error})))
    }
}
