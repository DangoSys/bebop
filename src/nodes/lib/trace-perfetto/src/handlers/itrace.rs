...
pub fn on_itrace(
  ...
  match event {
    "alloc" => on_alloc(state, obj, line_no, ts, rob_id, domain_id, funct, bank_enable, bank, pc, name),
...
fn on_alloc(
  ...
  state.open_rob.insert(rob_id, OpenRob { domain_id, name: name.clone() });
...
fn on_complete(
  ...
  let open = state.open_rob.remove(&rob_id).ok_or_else(|| ConvertError::InvalidLine {
    line: line_no,
  ...
  if open.domain_id != domain_id {
    return Err(ConvertError::InvalidLine {
      line: line_no,
      msg: format!("itrace complete domain mismatch for rob_id={rob_id}: alloc={} complete={}", open.domain_id, domain_id),
    });
  }
  state.events.push(json!({
