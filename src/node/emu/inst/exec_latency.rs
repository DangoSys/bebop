...
use super::decode::{
    self, FUNCT_BFP, FUNCT_DEQUANT, FUNCT_GEMMINI_COMPUTE_ACCUMULATED,
...
pub fn inst_cycles(funct: u32, xs1: u64, xs2: u64) -> u64 {
    match funct {
        ...
        f if f >= decode::FUNCT_GEMMINI_LOOP_WS_CONFIG_BOUNDS
            && f <= decode::FUNCT_GEMMINI_LOOP_WS =>
        {
            f80_gemmini_loop_ws::latency(funct, xs1, xs2)
        }
        f if f >= decode::FUNCT_GEMMINI_LOOP_CONV_WS_CONFIG_1
