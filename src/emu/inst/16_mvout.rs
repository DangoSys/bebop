use super::super::bank::{mem_write, BankConfig, BANK_NUM, BANK_SIZE};
use super::decode::{rs1_b0, rs1_iter, xs2_mem_stride};

pub fn exec(xs1: u64, xs2: u64, memory: &mut [u8], banks: &[Vec<u8>], cfgs: &[BankConfig]) -> u64 {
    ...
    for i in 0..depth {
        ...
        for j in 0..line_bytes {
            mem_write(memory, addr + j as u64, banks[bi][bank_offset + j]);
        }
    }
    0
}
